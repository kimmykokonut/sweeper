...
export default function ScoreBoard({
  ...
  const isLeaderTied =
    ...
    currentTotals[sortedPlayers[0]?.id] === currentTotals[sortedPlayers[1]?.id] &&
    highestScore > 0;
  ...
  return (
    <div className="mx-auto w-full max-w-4xl flex-1 flex flex-col px-3 sm:px-6 py-3 sm:py-4 space-y-4 sm:space-y-6 text-white min-h-0">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-emerald-800 pb-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-emerald-200">
            Playing to <span className="font-bold text-yellow-300">{settings.targetScore} points</span>
      ...
      {isFinished && winner && (
        <div className="rounded-2xl border-2 border-yellow-400 bg-linear-to-r from-emerald-950 via-emerald-900 to-emerald-950 p-5 shadow-2xl text-center space-y-3 animate-fade-in">
          ...
          <p className="text-emerald-200 text-sm">
            Victory achieved in {rounds.length} {rounds.length === 1 ? "round" : "rounds"} with{" "}
            <span className="font-bold text-yellow-300">{currentTotals[winner.id]} points</span> (target:{" "}
      ...
      {/* Player Score Badges */}
      <div className={`grid gap-3 ${players.length === 2 ? "grid-cols-2" : players.length === 3 ? "grid-cols-3" : "grid-cols-2 sm:grid-cols-4"}`}>
        {players.map((p) => {
          const score = currentTotals[p.id] || 0;
          const isLeader = score === highestScore && highestScore > 0 && !isLeaderTied;
          const progressPercent = Math.min(100, Math.round((score / settings.targetScore) * 100));
            ...
            >
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="flex items-center justify-between text-[11px] text-emerald-300 font-medium">
                  <span>🧹 {totalScope[p.id] || 0} scope</span>
                  <span className="inline-flex items-center gap-1">
                    <img src={coinIcon} alt="Denari" className="size-3.5 object-contain inline shrink-0" />
                    <span>{totalSettebello[p.id] || 0} settebello</span>
                  </span>
                </div>
              </div>
            </div>
        ...
        {rounds.length === 0 ? (
          <div className="p-8 text-center text-emerald-300 space-y-2">
            ...
            <p className="text-xs text-emerald-400">
              Deal the 40 cards, play the hand, and click "+ Score Round 1" above to record the score.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-emerald-800/80 overflow-x-auto">
            {rounds.map((round) => (
              ...
              >
                ...
                <div className="flex items-center justify-between sm:justify-start gap-3 sm:w-28 shrink-0">
                  ...
                  <div className="flex items-center gap-1 sm:hidden">
                    <button
                      ...
                      className="p-1 text-emerald-300 hover:text-white rounded hover:bg-emerald-800 text-xs"
                      title="Edit Round"
                    >
                      ✏️
                    </button>
                    <button
                      type="button"
                      onClick={() => setDeletingRoundNum(round.roundNumber)}
                      className="p-1 text-red-300 hover:text-red-100 rounded hover:bg-red-900/60 text-xs"
                      title="Delete Round"
                    >
                      🗑️
                    </button>
                      ...
                      >
                        <div className="flex items-center justify-between border-b border-emerald-800/50 pb-1 mb-1.5">
                          ...
                          <span className="font-black text-yellow-300 text-sm">
                            +{pts} <span className="text-[10px] text-emerald-400 font-normal">({cumTotal})</span>
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {gotSettebello && (
                            <span className="rounded bg-amber-400/20 text-yellow-300 border border-yellow-400/40 px-1.5 py-0.5 text-[10px] font-bold inline-flex items-center gap-1">
                              <img src={coinIcon} alt="Denari" className="size-3 object-contain inline shrink-0" />
                              <span>7 Bello</span>
                            </span>
                          )}
                          {gotPrimiera && (
                ...
                <div className="hidden sm:flex items-center gap-1.5 pl-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => onEditRound(round)}
                  ...
                  <button
                    type="button"
                    onClick={() => setDeletingRoundNum(round.roundNumber)}
                    className="p-1.5 text-red-300 hover:text-red-100 rounded-lg hover:bg-red-900/60 transition-colors text-sm"
                    title="Delete Round"
                  >
                    🗑️
                  </button>
      ...
      {deletingRoundNum !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-emerald-950 border border-red-700/80 p-5 shadow-2xl text-center space-y-4">
            <h4 className="text-lg font-bold text-red-300">
              Delete Round {deletingRoundNum}?
            </h4>
            <p className="text-xs text-emerald-200">
              This will remove Round {deletingRoundNum} and recalculate total scores for all remaining rounds.
            </p>
            <div className="flex gap-2 justify-center pt-2">
              <button
                type="button"
                onClick={() => setDeletingRoundNum(null)}
              ...
              <button
                type="button"
                onClick={() => {
                  onDeleteRound(deletingRoundNum);
                  setDeletingRoundNum(null);
                }}
                className="rounded-xl bg-red-600 px-4 py-2 font-bold text-white hover:bg-red-500 text-sm shadow-md"
              >
                Yes, Delete
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-3 sm:p-4">
          <div className="relative flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl bg-emerald-900 border border-emerald-700 p-5 shadow-2xl text-white overflow-y-auto space-y-4">
            <div className="flex items-center justify-between border-b border-emerald-800 pb-2">
              <h3 className="font-bold text-lg text-white">Scopa Scoring Rules</h3>
              <button
                type="button"
            ...
            <div className="text-xs sm:text-sm text-emerald-100 space-y-3">
              <p>
                Each round of Scopa deals all 40 cards. At the end of the round, 5 types of points can be scored:
              </p>

              <div className="space-y-2">
                <div className="rounded-lg bg-emerald-950/70 p-2.5 border border-emerald-800">
                  <span className="font-bold text-yellow-300">🧹 Scope (Sweeps): </span>
                  1 point each time a player captures all cards currently on the table during play.
                </div>

                <div className="rounded-lg bg-emerald-950/70 p-2.5 border border-emerald-800">
                  <span className="font-bold text-yellow-300">🃏 Carte (Cards): </span>
                  1 point to the player who captured more than 20 cards (21+). If tied (20-20), no point.
                </div>

                <div className="rounded-lg bg-emerald-950/70 p-2.5 border border-emerald-800 flex items-start gap-2">
                  <img src={setteBelloImg} alt="Settebello" className="h-8 w-auto rounded border border-yellow-400 shrink-0" />
                  <div>
                    <span className="font-bold text-yellow-300">⭐ Il Settebello: </span>
                    1 point to the player who captured the Seven of Coins (7 di Denari).
                  </div>
                </div>

                <div className="rounded-lg bg-emerald-950/70 p-2.5 border border-emerald-800">
                  <span className="font-bold text-yellow-300">🪙 Denari (Coins): </span>
                  1 point to the player who captured more than 5 coins (6+). If tied (5-5), no point.
                </div>

                <div className="rounded-lg bg-emerald-950/70 p-2.5 border border-emerald-800">
                  <span className="font-bold text-yellow-300">🏆 Primiera: </span>
                  1 point to the player with the highest Primiera score (best card in each suit: 7=21, 6=18, A=16, 5=15, 4=14, 3=13, 2=12, Face=10). You can use our built-in Primiera calculator!
                </div>
              </div>

              <div className="text-xs text-emerald-300 pt-1">
                <strong>Winning:</strong> The first player or team to reach or exceed {settings.targetScore} points with strictly the highest score wins the match!
              </div>
            </div>
